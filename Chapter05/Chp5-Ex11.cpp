...
public:
	...
	Student(string , string , char, float, string , const char*);
	...
	[[nodiscard]] const char* GetStudentId() const
	{
		return studentId;
	}
...
Student::Student(string  fn, string  ln, char mi, float avg, string  course, const char* id) :
	firstName(std::move(fn)), lastName(std::move(ln)), middleInitial(mi), gpa(avg), currentCourse(std::move(course))
{
