...
public:
	...
	void Print();
	void CleanUp();
	[[nodiscard]] float GetGpa() const; // access function for private data member
...
Student::Student(const string& fn, const string& ln, char mi, float avg, const char* course)
{
	...
	middleInitial = mi;
	gpa = avg;
	currentCourse =	new char[strlen(course) + 1];  // remember to dynamically allocate the memory for
...
int main()
{
	...
	e1.Print();
	s1.CleanUp();  // CleanUp() will delete() s1s' dynamically allocated data members.
