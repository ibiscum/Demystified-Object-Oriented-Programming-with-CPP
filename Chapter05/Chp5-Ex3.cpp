...
class Student
{
// private members are accessible only within the scope of the class (e.g. within member functions or friends)
private:
	string firstName; // data members
	string lastName;
	char middleInitial{};
	float gpa{};
	char* currentCourse{}; // though we'll prefer std::string, this data member will help us illustrate a few
	// important ideas public members are accessible from any scope.
...
{
	// even though string data members, firstName and lastName, are initialized with empty strings
	// (string is a class and does so for us), we're showing here how to clear the string, as it
	// may be re-initialized more than once.
	firstName.clear();
	lastName.clear();
...
void Student::Initialize(string fn, string ln, char mi, float avg, const char* course)
{
	...
	middleInitial = mi;
	gpa = avg;
	currentCourse = new char[strlen(course) + 1]; // remember to dynamically allocate the memory for data
