...
private:
	...
	char middleInitial;
	float gpa;
	string currentCourse;  // let's finally convert this to a std::string -- we've illustrated most
	// ptr special needs within classes by now
...
Student::Student(string fn, string ln, char mi, float avg, string  course, int id) :
	firstName(std::move(fn)), lastName(std::move(ln)), middleInitial(mi), gpa(avg), currentCourse(std::move(course)), studentId(id)
{
